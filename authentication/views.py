...
def log_in(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "authentication/homepage.html",
                          {'first_name': first_name, 'last_name': last_name, 'adm_no': adm_no})
...
# Checking whether student is logged in while trying to access marks page
def check_sessions(request):
    if request.user.is_authenticated:
        fname = request.session['first_name']
        number = request.session['adm_no']
        request.session.set_expiry(6000)
        return render(request, "authentication/career_choice.html", {'f_name': fname, 'num': number})
    else:
...
def check_marks(request):
    ...
    if request.method == 'POST':
        ...
        all_subjects = {
            'Subject': ['MAT', 'ENG', 'KIS', 'BIO', 'CHEM', 'PHY', 'BIED', 'GEO', 'HIS', 'REL', 'MUS', 'AGR', 'COM',
                        'HOM'],
        ...
        for index, rows in df.iterrows():
            if rows[1] > 0:
                ...
                elligible_reqs = elligible_course.requirements
                # 'e_cn':elligible_course_name,
        return render(request, "authentication/career_marks.html",{'average': average, 'grade': grade, 'e_cn': elligible_course_name, 'e_cd': elligible_course_desc,'e_reqs': elligible_reqs, 'f_name': fname, 'num': number})
